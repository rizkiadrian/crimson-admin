...
export default function ReferralCampaignEditPage() {
  ...
  return <CampaignEditForm initialData={campaign} />;
}
...
function CampaignEditForm({
  ...
  initialData: IReferralCampaignDetail;
}) {
  ...
  // Check if campaign has active referrals (simplified: if status is active/paused)
  const hasActiveReferrals =
    initialData.status === "active" || initialData.status === "paused";
  ...
  const [formData, setFormData] = useState<CampaignFormData>({
    ...
    max_referrals_per_user: initialData.max_referrals_per_user
      ? String(initialData.max_referrals_per_user)
      : "",
  });
  ...
  const [milestones, setMilestones] = useState<MilestoneFormData[]>(
    initialData.milestones.map((m) => ({
      ...
      referrer_reward_amount: m.referrer_reward_amount
        ? String(m.referrer_reward_amount)
        : "",
      referrer_voucher_id: m.referrer_voucher_id
        ? String(m.referrer_voucher_id)
        : "",
      referee_reward_type: m.referee_reward_type,
      referee_reward_amount: m.referee_reward_amount
        ? String(m.referee_reward_amount)
        : "",
      referee_voucher_id: m.referee_voucher_id
        ? String(m.referee_voucher_id)
        : "",
    }))
  );
  ...
  const [tiers, setTiers] = useState<TierFormData[]>(
    initialData.tiers.map((t) => ({
      name: t.name,
      min_referrals: String(t.min_referrals),
      max_referrals: t.max_referrals ? String(t.max_referrals) : "",
      bonus_percentage: String(t.bonus_percentage),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resp = await referralCampaignsService.update(initialData.id, {
        name: formData.name,
      ...
      showNotification(
        resp.message || "Campaign berhasil diperbarui",
        "success"
      );
    ...
    <div className="w-full">
      <FormCard>
        <FormCardHeader
          ...
          description="Perbarui konfigurasi campaign referral."
          badge="Referral Program"
        />

        <form onSubmit={handleSubmit}>
          <FormCardBody>
            {/* Warning notice when campaign has active referrals */}
            {hasActiveReferrals && (
              <div className="rounded-lg border border-yellow-300 bg-yellow-50 px-4 py-3">
                <p className="text-sm font-medium text-yellow-800">
                  ⚠️ Campaign ini memiliki referral aktif. Target role tidak
                  dapat diubah.
                </p>
              </div>
            )}
