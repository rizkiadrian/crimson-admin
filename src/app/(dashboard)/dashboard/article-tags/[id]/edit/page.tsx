...
export default function ArticleTagEditPage() {
  ...
  const fetcher = useCallback(
    () => articleTagsService.detail(Number(id)),
    [id]
  );
  const { data, isLoading, error } = useDetailData<IArticleTag>({
    fetcher,
    enabled: !!id,
  });
  ...
  return <TagEditForm initialData={data} />;
}

function TagEditForm({ initialData }: { initialData: IArticleTag }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const resp = await articleTagsService.update(initialData.id, { name });
