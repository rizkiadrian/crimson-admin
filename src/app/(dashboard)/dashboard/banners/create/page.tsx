...
/** Default values for a new text element */
function createDefaultTextElement(): ITextElement {
  return {
...
export default function BannerCreatePage() {
  ...
  const [backgroundConfig, setBackgroundConfig] =
    useState<IBackgroundConfig>(DEFAULT_BACKGROUND);
  const [selectedElementId, setSelectedElementId] = useState<string | null>(
    null
  );
  const [ctaConfig, setCtaConfig] = useState<ICtaConfig | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (bannerType === "image") {
        if (!imageFile) {
          setFormErrors({ image: "File gambar wajib diupload." });
          setSubmitting(false);
          return;
        }
        ...
        const resp = await bannersService.create(formData);
  ...
  return (
    <div className="w-full">
      <FormCard>
        <FormCardHeader
          title="Create Banner"
          description="Create a new banner for the mobile app. Choose between uploading an image or designing with the text placement editor."
        ...
        <form onSubmit={handleSubmit}>
          <FormCardBody>
            ...
            {bannerType === "image" && imagePreviewUrl && (
              <div className="mt-4">
                <Button
                  type="button"
                  variant="ghost"
      ...
      <BannerPreviewModal
        ...
        banner={{
          type: bannerType,
          image: bannerType === "image" ? imageFile : null,
          backgroundConfig:
