...
export default function VoucherEditPage() {
  ...
  return <VoucherEditForm initialData={voucher} />;
}

// ─── Inner form (pre-filled, per ADR-01) ─────────────────────────────────────

function VoucherEditForm({ initialData }: { initialData: IVoucher }) {
  const router = useRouter();
  const showNotification = useNotificationStore((s) => s.showNotification);

  const hasBeenUsed = initialData.used_count > 0;
  ...
  const [formData, setFormData] = useState<VoucherFormData>({
    ...
    max_discount_cap: initialData.max_discount_cap
      ? String(initialData.max_discount_cap)
      : "",
    ...
    min_transaction_amount: initialData.min_transaction_amount
      ? String(initialData.min_transaction_amount)
      : "",
    distribution_type: initialData.distribution_type,
    target_user_type: initialData.target_user_type,
    segment_type: "all",
    user_ids: "",
  });
  ...
  return (
    <div className="w-full">
      <FormCard>
        <FormCardHeader
          title="Edit Voucher"
          description="Perbarui konfigurasi voucher. Beberapa field tidak dapat diubah jika voucher sudah pernah digunakan."
          badge="Voucher Management"
        />

        <form onSubmit={handleSubmit}>
          <FormCardBody>
            {/* Warning notice when voucher has been used */}
            {hasBeenUsed && (
              <div className="rounded-lg border border-yellow-300 bg-yellow-50 px-4 py-3">
                <p className="text-sm font-medium text-yellow-800">
                  ⚠️ Voucher ini sudah digunakan ({initialData.used_count}x).
                  Tipe diskon dan kode voucher tidak dapat diubah.
                </p>
              </div>
            )}
